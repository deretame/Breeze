pub(crate) fn fs_task_dispatch(op: String, args_json: String) -> String {
    ...
    let arg_str = |idx: usize, name: &str| -> Result<String, String> {
        args.get(idx)
            ...
            .ok_or_else(|| crate::tr!("argument-must-be-a-string", name = name))
    };
    let arg_bool = |idx: usize, name: &str| -> Result<bool, String> {
        args.get(idx)
            .and_then(Value::as_bool)
            .ok_or_else(|| crate::tr!("argument-must-be-a-boolean", name = name))
    };
    let arg_u64 = |idx: usize, name: &str| -> Result<u64, String> {
        args.get(idx)
            .and_then(Value::as_u64)
            .ok_or_else(|| crate::tr!("argument-must-be-a-non-negative-integer", name = name))
    };
    let arg_u32 = |idx: usize, name: &str| -> Result<u32, String> {
        arg_u64(idx, name).and_then(|v| {
            u32::try_from(v).map_err(|_| crate::tr!("argument-exceeds-u32-range", name = name))
        })
    };
    let arg_i64 = |idx: usize, name: &str| -> Result<i64, String> {
        args.get(idx)
            .and_then(Value::as_i64)
            .ok_or_else(|| crate::tr!("argument-must-be-an-integer", name = name))
    };
    ...
    match op.as_str() {
        ...
        "mkdir" => match (arg_str(0, "path"), arg_bool(1, "recursive")) {
            ...
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("mkdir-argument-invalid") }).to_string(),
        },
        "readdir" => match (arg_str(0, "path"), arg_bool(1, "withFileTypes")) {
            (Ok(path), Ok(with_file_types)) => fs_readdir(path, with_file_types),
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("readdir-argument-invalid") }).to_string(),
        },
        ...
        "rm" => match (
            ...
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("rm-argument-invalid") }).to_string(),
        },
        "rename" => match (arg_str(0, "oldPath"), arg_str(1, "newPath")) {
            (Ok(old_path), Ok(new_path)) => fs_rename(old_path, new_path),
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("rename-argument-invalid") }).to_string(),
        },
        "copyFile" => match (arg_str(0, "src"), arg_str(1, "dst")) {
            (Ok(src), Ok(dst)) => fs_copy_file(src, dst),
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("copyfile-argument-invalid") }).to_string(),
        },
        "cp" => match (
            arg_str(0, "src"),
            arg_str(1, "dst"),
            ...
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("cp-argument-invalid") }).to_string(),
        },
        ...
        "symlink" => match (
            ...
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("symlink-argument-invalid") }).to_string(),
        },
        "link" => match (arg_str(0, "existingPath"), arg_str(1, "newPath")) {
            (Ok(existing_path), Ok(new_path)) => fs_link(existing_path, new_path),
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("link-argument-invalid") }).to_string(),
        },
        "truncate" => match (arg_str(0, "path"), arg_u64(1, "len")) {
            (Ok(path), Ok(len)) => fs_truncate(path, len),
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("truncate-argument-invalid") }).to_string(),
        },
        "chmod" => match (arg_str(0, "path"), arg_u32(1, "mode")) {
            (Ok(path), Ok(mode)) => fs_chmod(path, mode),
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("chmod-argument-invalid") }).to_string(),
        },
        "utimes" => match (arg_str(0, "path"), arg_i64(1, "atime"), arg_i64(2, "mtime")) {
            ...
            _ => json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("utimes-argument-invalid") }).to_string(),
        },
        ...
        _ => {
            json!({ "ok": false, "code": "EINVAL", "error": crate::tr!("unsupported-fs-async-operation", op = op) })
                .to_string()
        }
    }
}
        ...
        Ok(bytes) => {
            ...
            if encoding.is_empty() {
                ...
                json!({
                    "ok": false,
                    "code": "EINVAL",
                    "error": crate::tr!("unsupported-encoding", encoding = encoding)
                })
                .to_string()
            }
        }
...
fn parse_fs_write_payload(data_json: String, encoding: Option<String>) -> Result<Vec<u8>, String> {
    ...
    let kind = value
        .get("kind")
        .and_then(Value::as_str)
        .ok_or(crate::tr!("missing-kind-field"))?;

    if kind == "bytes" {
        let list = value
            .get("data")
            .and_then(Value::as_array)
            .ok_or(crate::tr!("bytes-data-format-error"))?;
        let mut out = Vec::with_capacity(list.len());
        for item in list {
            let num = item
                .as_u64()
                .ok_or(crate::tr!("bytes-data-must-be-integers-in-0-255"))?;
            if num > 255 {
                return Err(crate::tr!("bytes-data-must-be-in-the-0-255-range").to_string());
            }
            out.push(num as u8);
        }
        return Ok(out);
    ...
    if kind == "text" {
        let text = value
            .get("data")
            .and_then(Value::as_str)
            .ok_or(crate::tr!("text-data-format-error"))?;
        let encoding = normalize_encoding(encoding);
        if encoding.is_empty() || encoding == "utf8" || encoding == "utf-8" {
            return Ok(text.as_bytes().to_vec());
        }
        return Err(crate::tr!("unsupported-encoding", encoding = encoding));
    }

    Err(crate::tr!("unsupported-kind", kind = kind))
}
...
pub fn fs_rm(path: String, recursive: bool, force: bool) -> String {
    ...
    if !target.exists() {
        ...
        return json!({ "ok": false, "code": "ENOENT", "error": crate::tr!("file-or-directory-does-not-exist") }).to_string();
    }
...
fn create_symlink_impl(_target: &str, _path: &str, _is_dir: bool) -> io::Result<()> {
    Err(io::Error::new(
        ...
        crate::tr!("symbolic-links-are-not-supported-on-this-platform"),
    ))
}
...
fn chmod_impl(_path: &str, _mode: u32) -> io::Result<()> {
    Err(io::Error::new(
        io::ErrorKind::Unsupported,
        crate::tr!("chmod-is-not-supported-on-this-platform"),
    ))
}
...
pub fn fs_cp(
    ...
    if !src_path.exists() {
        return json!({ "ok": false, "code": "ENOENT", "error": crate::tr!("source-path-does-not-exist") })
            .to_string();
    }
    if dst_path.exists() {
        if error_on_exist {
            return json!({ "ok": false, "code": "EEXIST", "error": crate::tr!("destination-path-already-exists") }).to_string();
        }
        if !force {
            return json!({ "ok": false, "code": "EEXIST", "error": crate::tr!("destination-path-already-exists-and-force-is-not") }).to_string();
        }
    }
    ...
    let result = if src_path.is_dir() {
        if !recursive {
            Err(io::Error::new(
                ...
                crate::tr!("recursive-must-be-enabled-when-copying-a-directory"),
            ))
        } else {
            copy_dir_recursive(src_path, dst_path)
        }
...
pub fn fs_mkdtemp(prefix: String) -> String {
    ...
    json!({ "ok": false, "code": "EEXIST", "error": crate::tr!("cannot-create-unique-temporary-directory") })
        .to_string()
}
use super::*;
