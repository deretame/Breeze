  ...
  try {
    ...
    case 129382005: {
      ...
      auto task = [](std::shared_ptr<Session> session, std::uint64_t gen,
                     std::uint64_t req, std::uint32_t method,
                     std::string runtime_name) -> stdexec::task<void> {
        try {
    ...
    case 381890690: {
      ...
      auto task = [](std::shared_ptr<Session> session, std::uint64_t gen,
                     std::uint64_t req, std::uint32_t method,
                     std::string runtime_name, std::string bundle_name,
                     std::string bundle_js) -> stdexec::task<void> {
        try {
    ...
    case 760810845: {
      ByteReader r(frame.payload.data(), frame.payload.size());
      const auto function_name = r.str();
      ...
      ByteWriter w;
      {
        auto out = ::qjs_register_function(function_name, callback);
        w.u8(out ? 1 : 0);
      }
      post_ok(session, gen, req, method, w.raw());
      break;
    ...
    case 1159358141: {
      ...
      auto task = [](std::shared_ptr<Session> session, std::uint64_t gen,
                     std::uint64_t req, std::uint32_t method,
                     std::string runtime_name, std::string task_group_key,
                     bool is_once, std::optional<std::string> bundle_js,
                     std::string fn_path,
                     std::string args_json) -> stdexec::task<void> {
        try {
          auto out =
              co_await ::qjs_task_call(runtime_name, task_group_key, is_once,
                                       bundle_js, fn_path, args_json);
          ByteWriter w;
          w.u8vec(out);
          post_ok(session, gen, req, method, w.raw());
        } catch (const std::exception &e) {
        ...
        co_return;
                                            ...
                                            std::move(task_group_key), is_once,
                                            bundle_js, std::move(fn_path),
                                            std::move(args_json));
      spawn_on_io(std::move(task));
      break;
    }
    ...
    case 1340509605: {
      ...
      ByteWriter w;
      {
        auto out = ::qjs_unregister_function(function_name);
        w.u8(out ? 1 : 0);
      }
      post_ok(session, gen, req, method, w.raw());
      break;
...
std::vector<std::uint8_t> dispatch_sync(std::uint64_t session_id,
  ...
  if (frame.method_id == 760810845u) {
    ...
    try {
      ...
      ByteWriter ew;
      ew.i32(1);
      ew.str(dcb::error::format("qjs_register_function", e.what()));
      return make_frame(MsgType::kResponseErr, frame.request_id,
                        frame.method_id, ew.raw());
    } catch (...) {
      ByteWriter ew;
      ew.i32(1);
      ew.str(dcb::error::format("qjs_register_function", "unknown"));
      return make_frame(MsgType::kResponseErr, frame.request_id,
                        frame.method_id, ew.raw());
    }
  }
  ...
  if (frame.method_id == 1340509605u) {
    ByteReader r(frame.payload.data(), frame.payload.size());
    const auto function_name = r.str();
    try {
      ByteWriter w;
      ...
      ByteWriter ew;
      ew.i32(1);
      ew.str(dcb::error::format("qjs_unregister_function", e.what()));
      return make_frame(MsgType::kResponseErr, frame.request_id,
                        frame.method_id, ew.raw());
    } catch (...) {
      ByteWriter ew;
      ew.i32(1);
      ew.str(dcb::error::format("qjs_unregister_function", "unknown"));
      return make_frame(MsgType::kResponseErr, frame.request_id,
                        frame.method_id, ew.raw());
    }
  }
