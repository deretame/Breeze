...
fn unique_temp_dir(prefix: &str) -> PathBuf {
    ...
    std::fs::create_dir_all(&dir).expect(&crate::tr!("failed-to-create-temporary-directory"));
    dir
}
...
fn fs_only_async_apis_exposed() {
    ...
    let result = run_async_script_with_fs(script).expect(&crate::tr!("failed-to-execute-script"));
    let parsed: Value = serde_json::from_str(&result).expect(&crate::tr!("failed-to-parse-result"));
...
fn fs_mkdir_readdir_stat() {
    ...
    let names = parsed["names"]
        .as_array()
        .expect(&crate::tr!("names-must-be-an-array"));
