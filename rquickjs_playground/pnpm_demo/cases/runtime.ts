import { getApi, hostRuntime, requireCryptoLike } from "../types/runtime-api";
...
export default async function main() {
  ...
  const pathRef: { join: (...parts: string[]) => string } =
    runtimePath && typeof runtimePath.join === "function"
      ...
      : {
          join: (...parts: string[]) =>
            parts.join("/").replace(/\/+/g, "/").replace("/b/../", "/"),
        };
  ...
  const b = BufferRef.concat([
    BufferRef.from("ab"),
    BufferRef.from("cd"),
  ]).toString("utf8");

  const id = String(hostRuntime.uuidv4() || "");
  const uuidOk =
    /^[0-9a-f]{32}$/.test(id) ||
