  ...
  Widget _changeProfilePicture() {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        debugPrint('Selected image path: $selectedImages'); // 输出选择的图片路径
        File originalFile = File(selectedImages);
        List<int> imageBytes = await originalFile.readAsBytes();

        var decodedImage = decodeImage(Uint8List.fromList(imageBytes));

        List<int> compressedBytes = encodeJpg(decodedImage!, quality: 50);
