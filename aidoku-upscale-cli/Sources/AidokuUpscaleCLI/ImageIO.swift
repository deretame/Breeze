...
enum ImageIO {
    static func loadCGImage(from path: String) throws -> CGImage {
        ...
        guard let data = try? Data(contentsOf: url) else {
            throw CLIError.invalidInput("Cannot read file at \(path)")
        }
        guard let source = CGImageSourceCreateWithData(data as CFData, nil),
              let image = CGImageSourceCreateImageAtIndex(source, 0, nil)
        else {
            throw CLIError.invalidInput("Cannot decode image at \(path)")
        }
        return image
    }
    ...
    static func writeCGImage(_ image: CGImage, to path: String) throws {
        ...
        guard let destination = CGImageDestinationCreateWithURL(url as CFURL, uti, 1, nil) else {
            throw CLIError.writeFailed("Cannot create image destination at \(path)")
        }
        CGImageDestinationAddImage(destination, image, nil)
        if !CGImageDestinationFinalize(destination) {
            throw CLIError.writeFailed("Failed to write image to \(path)")
        }
    }
}
