import { getApi, requireApi, requireCryptoLike } from "../types/runtime-api";
...
export default async function main(config: unknown = {}) {
  ...
  const nativeOut = await native.chain(
    ["invert", "invert"],
  ...
  const fileText =
    typeof fileRaw === "string" ? fileRaw : new TextDecoder().decode(fileRaw);
